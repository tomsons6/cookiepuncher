...
namespace BNG
{
    ...
    public class VRUISystem : BaseInputModule
    {
        ...
        public override void Process ()
        {
            ...
            {
                // A check so that when moving cursor off the current UI object, it doesn't stay in the pressed state. With this, when pressing on object, but moving the cursor away before releasing, the UI object returns to the normal state.
                // If any unusual errors appear where they shouldn't, this is probably the cause.
                if (EventData.pointerCurrentRaycast.gameObject != null && ReleasingObject == EventData.pointerCurrentRaycast.gameObject)
                {
                    Press ();
                }
